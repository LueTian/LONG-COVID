...
comornet['source'] = comornet['source'].apply(Int2Str)
comornet['target'] = comornet['target'].apply(Int2Str)
comornet['edge'] = comornet['source'].str.cat(comornet['target'],sep='_')
comornet = comornet[(comornet['source'].isin(ICD_ls))&(comornet['target'].isin(ICD_ls))].reset_index(drop = True) # 230 

ICD_pair = [[i,j] for i in ICD_ls for j in ICD_ls]
ICD_pair = pd.DataFrame(ICD_pair,columns=['source','target'])
ICD_pair['edge'] = ICD_pair['source'].str.cat(ICD_pair['target'],sep='_') # 288369
ICD_pair = ICD_pair[ICD_pair['source']!=ICD_pair['target']].reset_index(drop = True) # 287832
ICD_pair = ICD_pair[ICD_pair['source'].isin(comornet['source'])].reset_index(drop = True) # 42344 
ICD_pair_T = ICD_pair[ICD_pair['edge'].isin(comornet['edge'])]# 230
ICD_pair_F = ICD_pair.drop(index = ICD_pair_T.index)# 42114

def ICD_str(input):
    outcome = 'ICD ' + input
    return outcome

ICD_pair_T['source'] = ICD_pair_T['source'].apply(ICD_str)
ICD_pair_T['target'] = ICD_pair_T['target'].apply(ICD_str)
ICD_pair_F['source'] = ICD_pair_F['source'].apply(ICD_str)
ICD_pair_F['target'] = ICD_pair_F['target'].apply(ICD_str)

ICD_LCC = pd.read_csv(r'Data\Code_02_Outcome\ICD_LCC.csv')
ICD_LCC['ICDID'] = ICD_LCC['ICDID'].apply(Int2Str)
ICD_LCC['ICDID'] = ICD_LCC['ICDID'].apply(ICD_str)
ICD_LCC.to_csv(r'Data\Code_02_Outcome\ICD_LCC.csv',index = False)
ICD_pair_T.to_csv(r'Data\Code_02_Outcome\ICD_pair_T.csv',index = False)
ICD_pair_F.to_csv(r'Data\Code_02_Outcome\ICD_pair_F.csv',index = False)
