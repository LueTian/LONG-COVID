'''
permutation test for positive sample
'''
# packages
import pandas as pd
import numpy as np
...
### Data Load 
pairs = pd.read_csv(r"Data\Code_02_Outcome\ICD_pair_T.csv") # 271
pairs['source'] = pairs['source'].str.split(' ').str.get(1)
pairs['target'] = pairs['target'].str.split(' ').str.get(1)
...
for idx in pairs.index:
    repeat_ls = []
    edge = pairs.loc[idx,'edge']
    source_disease = pairs.loc[idx,'source']
    target_disease = pairs.loc[idx,'target']
    source_protein = ICD_Protein_Map[source_disease]
    target_protein = ICD_Protein_Map[target_disease]
    epoch_dis_ls = []
    for epoch in range(repeat_num):
        G_S_s,G_T_s = Random_Select_Result(source_protein,target_protein,node_degree_df,bins)
        repeat_ls.append([edge,epoch,str(sorted(list(G_S_s))),str(sorted(list(G_T_s)))])
    repeat_ls = pd.DataFrame(repeat_ls,columns=['edge','epoch','source_protein','target_protein'])
    repeat_ls.to_csv(f"Data\\Code_04_Outcome\\{edge}_repeat.csv",index = False)
    print(edge)
