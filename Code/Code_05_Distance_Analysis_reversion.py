...
pvalues[pvalues['pvalue'] < 0.05].shape # (54, 7)
...
Pair_T= pd.read_csv("Data\\Code_03_Outcome\\ICD_pair_T_add(2).csv")
Pair_T['distance (DIFF)'] = Pair_T['outcome1'] - Pair_T['outcome1_add']
Pair_T = pd.merge(Pair_T, Ref_DF, how = 'left', on = 'edge')

Pair_T[['z_score (distance (DIFF))','p_value (distance (DIFF))']] = Pair_T[['distance (DIFF)','mean distance (DIFF)','std distance (DIFF)','values distance (DIFF)']].apply(Z_Score,axis = 1,result_type='expand')
def Test1(input,str1 = 'less'):
    list1 = eval(input[0])
    value1 = input[1]
    statistic,pvalue = wilcoxon(list1 , [value1] * len(list1), alternative= str1)
    return pvalue

def Test2(input,str1 = 'less'):
    list1 = eval(input[0])
    value1 = input[1]
    statistic,pvalue = ttest_1samp(list1 , value1, alternative= str1)
    return pvalue

# Pair_T['p_value (distribution) (distance (DIFF))'] = Pair_T[['values distance (DIFF)', 'distance (DIFF)']].apply(Test1, axis = 1)
# Pair_T[Pair_T['p_value (distribution) (distance (DIFF))'] < 0.05] # 47
Pair_T['p_value (distribution) (distance (DIFF))'] = Pair_T[['values distance (DIFF)', 'distance (DIFF)']].apply(Test2, axis = 1)
Pair_T[Pair_T['p_value (distribution) (distance (DIFF))']< 0.05] # 45
...
[Z_socres_significant.shape, Z_socres_total.shape] # [(45, 6), (145, 6)]
[Z_scores_total_positive.shape, Z_scores_total_negative.shape] # [(46, 6), (99, 6)]
...
DF = pd.read_csv(r"F:\Research Code Backup\Deciphering the Molecular Mechanism of Post-Acute Sequelae of COVID-19 through Comorbidity Network Analysis\Supplemtary Material\DF_S7.csv")
DF['p_value (distribution) (distance (DIFF))'] = DF['p_value (distribution) (distance (DIFF))'].round(8)
DF.to_csv(r"F:\Research Code Backup\Deciphering the Molecular Mechanism of Post-Acute Sequelae of COVID-19 through Comorbidity Network Analysis\Supplemtary Material\DF_S7.csv",index = False)

DF = pd.read_csv(r"F:\Research Code Backup\Deciphering the Molecular Mechanism of Post-Acute Sequelae of COVID-19 through Comorbidity Network Analysis\Data\Code_05_Outcome\z_score_reversion.csv")
DF['p_value (distribution) (distance (DIFF))'] = DF['p_value (distribution) (distance (DIFF))'].round(8)
DF.to_csv(r"F:\Research Code Backup\Deciphering the Molecular Mechanism of Post-Acute Sequelae of COVID-19 through Comorbidity Network Analysis\Data\Code_05_Outcome\z_score_reversion.csv",index = False)