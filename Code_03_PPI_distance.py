...
ICD_pair_T = pd.read_csv(r"Data\Code_02_Outcome\ICD_pair_T.csv")
ICD_pair_F = pd.read_csv(r"Data\Code_02_Outcome\ICD_pair_F.csv")
...
ppi_net.PPI_dict = {i[0]:int(i[1]) for i in np.load('Data\Code_03_Outcome\PPI_dict.npy')}

### PPI distance before COVID infection
ICD_pair_T[['outcome1','outcome2','outcome3']] = ICD_pair_T[['source_portein','target_portein']].apply(ppi_net.group_group_distance,axis = 1,result_type='expand')
ICD_pair_F[['outcome1','outcome2','outcome3']] = ICD_pair_F[['source_portein','target_portein']].apply(ppi_net.group_group_distance,axis = 1,result_type='expand')
...
### PPI distance before COVID infection
ICD_pair_T['source_portein_add'] = [sorted(set(i+COVID_LCC['EntrezID'].tolist())) for i in ICD_pair_T['source_portein']]
ICD_pair_F['source_portein_add'] = [sorted(set(i+COVID_LCC['EntrezID'].tolist())) for i in ICD_pair_F['source_portein']]

ICD_pair_T[['outcome1_add','outcome2_add','outcome3_add']] = ICD_pair_T[['source_portein_add','target_portein']].apply(ppi_net.group_group_distance,axis = 1,result_type='expand')
ICD_pair_F[['outcome1_add','outcome2_add','outcome3_add']] = ICD_pair_F[['source_portein_add','target_portein']].apply(ppi_net.group_group_distance,axis = 1,result_type='expand')
...
len(ppi_net.PPI_dict.keys()) # 16557019
array = np.array(list(ppi_net.PPI_dict.items()))
np.save(r"Data\Code_03_Outcome\PPI_dict.npy",array)
